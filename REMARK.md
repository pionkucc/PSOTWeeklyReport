...
Excel配置的路径格式为`/待协调事项_2026.06.08.html`和`/UI自动化场景进度.jpg`，在不同操作系统下行为不一致：

**Windows环境**（本地）：
```python
os.path.isabs('/file.jpg')  # 返回 False（绝对路径需要盘符如 C:\）
# 路径被正确处理为相对路径，文件能找到
```

**Linux环境**（GitHub Actions）：
```python
os.path.isabs('/file.jpg')  # 返回 True（/开头是绝对路径）
# 路径被当作绝对路径，指向根目录，导致文件找不到
```

### 12.2 图片路径修复

修复`data_processor.py`第102-107行：
...
修复`views/home_view.py`第1098-1105行：
...
去除行首序号文字（如`1、`、`2、`），使用SVG图标替代：

```python
# views/home_view.py - _render_next_week_plan
import re
# 去除行首的序号（如 1、 2、 1. 2. 等）
