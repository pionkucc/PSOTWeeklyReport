...
**更新内容**：
- 跨平台路径处理修复（解决GitHub Actions构建问题）
  - 修复`data_processor.py`图片路径处理：移除`os.path.isabs()`判断
  - 修复`views/home_view.py`HTML文件引用路径处理：同样移除绝对路径判断
  - 统一将以`/`开头的路径视为相对路径（项目根目录）
  - 解决Linux环境下`os.path.isabs('/file')`误判为绝对路径的问题
- 下周测试计划显示优化
  - 去除行首序号文字（如`1、`、`2、`、`1.`等）
  - 使用SVG同心圆图标替代序号，视觉效果更统一
  - 同时处理富文本HTML和纯文本两种情况

**修复问题**：
- GitHub Actions部署后待协调事项章节为空（HTML文件找不到）
- GitHub Actions部署后UI自动化场景进度图片为空（图片路径错误）
- 下周测试计划显示序号文字冗余

**更新文件**：
- `data_processor.py` - 图片路径处理修复（第102-107行）
- `views/home_view.py` - HTML文件引用修复（第1098-1105行）+ 下周测试计划序号去除（第1039行、1056行）

**备份文件**：
- `versions/v3.5.4_modular_backup.zip`
...
**更新文件**：
- `views/home_view.py` - 剩余内容渲染 + 缺省页 + font-size清理

**备份文件**：
- `versions/v3.5.2_modular_backup.zip`
