...
def create_metrics_cards_html():
    ...
    cards = [
        ...
        ('关闭率', f'{close_rate:.1f}%', 'linear-gradient(135deg, #82E0AA 0%, #A8D8EA 100%)', 'icon-check', f'遗留缺陷{legacy_count}个'),
...
fig.update_xaxes(tickfont={'size': 11, 'family': 'Microsoft YaHei'})
fig.update_yaxes(gridcolor='#e8e8e8', griddash='dot')
trend_chart_html = fig.to_html(full_html=False, config=config)  # 单独存储，用于宽卡片
