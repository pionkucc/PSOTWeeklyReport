...
def create_pie_chart(data, title, hole=0.4, colors=None, height=350, legend_y=-0.2):
    ...
    fig.update_layout(
        ...
        hoverlabel=dict(
            bgcolor='white',
            bordercolor='#e0e0e0',
            font=dict(size=13, family='Microsoft YaHei', color='#333'),
            align='left',
            namelength=-1
        )
    )
...
<body>
    ...
    (function() {{
            ...
            document.querySelectorAll('.plotly-graph-div').forEach(function(graphDiv) {{
                graphDiv.on('plotly_hover', function(data) {{
                    ...
                    if (pt.fullData.type === 'pie') {{
                        ...
                        if (paths[pt.pointNumber]) {{
                            paths[pt.pointNumber].style.transform = 'scale(1.05)';
                            paths[pt.pointNumber].style.transformOrigin = 'center';
                            paths[pt.pointNumber].style.transition = 'transform 0.3s ease';
                        }}
                    }}
                }});
        ...
        if (typeof Plotly !== 'undefined') {{
            setTimeout(addPieHoverAnimation, 800);
        }} else {{
