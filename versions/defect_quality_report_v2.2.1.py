...
def create_horizontal_bar_with_legend(data, title, colors=None, chart_id="chart", order=None):
    """创建带自定义图例的横向柱状图，点击图例可隐藏柱子和Y轴标签"""
    counts = data.value_counts()
...
def create_detail_view_html():
    ...
    col_names_js = json.dumps(col_names, ensure_ascii=False)
...
<style>
    ...
    .detail-table td.summary-cell:hover {{
        white-space: normal;
        word-break: break-all;
        background: #fff;
        box-shadow: 0 2px 8px rgba(170,150,218,0.2);
        z-index: 10;
        position: relative;
        border-radius: 4px;
        padding: 10px;
    ...
    .modal-content {{
        ...
        max-height: 80vh;
        overflow: hidden;
        box-shadow: 0 12px 40px rgba(170,150,218,0.2);
        ...
        data.forEach(function(row, dataIdx) {{
            ...
            colKeys.forEach(function(key, idx) {{
                ...
                if (key === '缺陷状态') {{
                    ...
                    var cls = '';
                    if (val === '优先' || val === '高') cls = 'priority-high';
                    else if (val === '中') cls = 'priority-medium';
...
# 生成明细视图HTML
detail_view_html = create_detail_view_html()
...
# 添加堆叠柱状图
handler_totals = handler_status_counts.sum(axis=1).tolist()
    ...
    if i == len(status_list) - 1:
        bar.add_yaxis(
            ...
            label_opts=opts.LabelOpts(
                is_show=True,
                position="top",
                formatter=JsCode(f"function(p){{ var totals = {handler_totals}; return totals[p.dataIndex]; }}"),
                font_family="Microsoft YaHei",
                font_size=11,
                color="#2c3e50"
            )
        ...
        formatter=JsCode("""
            function(params) {
                ...
                params.forEach(function(item) {
                    if (item.seriesName === '平均修复时长(天)') {
                        var val = item.value[1] !== undefined ? item.value[1] : item.value;
                        result += item.marker + item.seriesName + ': ' + (val > 0 ? val.toFixed(2) : val) + '<br/>';
                    } else {
...
charts_html.append(create_horizontal_bar_with_legend(df['发现阶段'], '缺陷发现阶段分布', colors=stage_colors, chart_id="chart_stage", order=stage_order))
...
# 10. 缺陷引入原因分析 (横向柱状图，带自定义图例)
charts_html.append(create_horizontal_bar_with_legend(df['引入原因'].dropna(), '缺陷引入原因分析', colors=COLORS['pastel'], chart_id="chart_cause"))

# 11. 缺陷优先级分布 (柱状图，带自定义图例)
...
# 12. 缺陷严重程度分布 (横向柱状图，带自定义图例)
severity_order = ['严重', '一般', '轻微', '建议']
severity_colors = [COLORS['severity'].get(s, '#A8D8EA') for s in severity_order]
charts_html.append(create_horizontal_bar_with_legend(df['严重程度'], '缺陷严重程度分布', colors=severity_colors, chart_id="chart_severity", order=severity_order))

# 13. 缺陷引入阶段分布 (横向柱状图，带自定义图例)
charts_html.append(create_horizontal_bar_with_legend(df['引入阶段'].dropna(), '缺陷引入阶段分布', colors=COLORS['fresh'], chart_id="chart_intro"))
